...
const NVFormatInfo formatsInfo[] = {
    ...
    [NV_FORMAT_P010] = {2,
                        ...
                        true,
                        false,
                        {{1, DRM_FORMAT_R16, {0, 0}},
                         {2, DRM_FORMAT_RG1616, {1, 1}}},
    ...
    [NV_FORMAT_Q416] = {2,
                        ...
                        {{1, DRM_FORMAT_R16, {0, 0}},
                         {1, DRM_FORMAT_R16, {0, 0}},
                         {1, DRM_FORMAT_R16, {0, 0}}},
                        {VA_FOURCC_Q416, VA_LSB_FIRST, 48, 0, 0, 0, 0, 0}},
#endif
};
...
static void nvencDebugInitOnce(void) {
  ...
  if (mkdir(g_encDebugDir, 0775) != 0 && errno != EEXIST) {
    ...
    LOG("ENCDBG enabled dir=%s start_frame=%u max_frames=%u", g_encDebugDir,
        g_encDebugStartFrame, g_encDebugMaxFrames);
  }
  pthread_mutex_unlock(&g_encDebugMutex);
}

static void nvencDebugDumpNV12(const char *path, const uint8_t *y,
                               const uint8_t *uv, size_t pitch, uint32_t width,
                               uint32_t height) {
...
static void nvencDebugDumpBytes(const char *path, const uint8_t *data,
                                size_t size) {
  FILE *f = fopen(path, "wb");
  if (f == NULL) {
    LOG("ENCDBG: failed to open %s: %s", path, strerror(errno));
...
__attribute__((constructor)) static void init() {
  ...
  if (nvdEncStrict != NULL && nvdEncStrict[0] != '\0' &&
      strcmp(nvdEncStrict, "0") != 0) {
    g_encStrictMode = 1;
  }
  if (g_encStrictMode) {
  ...
  if (fd < 0) {
    ...
    LOG("If running in Firefox, set env var MOZ_DISABLE_RDD_SANDBOX=1 to "
        "disable sandbox.");
    // exit here so we don't init CUDA, unless an env var has been set to force
    // us to init even though we've detected a sandbox
...
#endif
void logger(const char *filename, const char *function, int line,
            const char *msg, ...) {
  if (LOG_OUTPUT == 0) {
    return;
  }
  ...
  fprintf(LOG_OUTPUT, "%10ld.%09ld [%d-%d] %s:%4d %24s %s\n", (long)tp.tv_sec,
          tp.tv_nsec, getpid(), nv_gettid(), filename, line, function,
          formattedMessage);
  fflush(LOG_OUTPUT);
}

bool checkCudaErrors(CUresult err, const char *file, const char *function,
                     const int line) {
  if (CUDA_SUCCESS != err) {
...
static Object allocateObject(NVDriver *drv, ObjectType type,
                             size_t allocatePtrSize) {
...
static bool destroyContext(NVDriver *drv, NVContext *nvCtx) {
  if (nvCtx->mode == NV_CONTEXT_ENCODE) {
    if (nvCtx->encFrameNum > 0 && nvCtx->encOutputFrameCount == 0) {
      nvencHealthLog("WARN",
                     "HW encode context closed without output frames (%ux%u); "
                     "client may fall back to software",
...
static bool doesGPUSupportCodec(cudaVideoCodec codec, int bitDepth,
                                cudaVideoChromaFormat chromaFormat,
                                uint32_t *width, uint32_t *height) {
  CUVIDDECODECAPS videoDecodeCaps = {.eCodecType = codec,
...
static bool profileToNvencGuids(VAProfile profile, GUID *codecGuid,
                                GUID *profileGuid, int *isHevc) {
  switch (profile) {
  case VAProfileH264ConstrainedBaseline:
...
static bool loadNvencFunctions(NVDriver *drv) {
  ...
  PFN_NvEncodeAPICreateInstance createInstance =
      (PFN_NvEncodeAPICreateInstance)dlsym(drv->nvencLib,
...
static void nvencDestroyIoBuffers(NVDriver *drv, NVContext *nvCtx) {
  ...
  for (uint32_t i = 0; i < NVENC_MAX_IO_BUFFERS; i++) {
    if (nvCtx->nvencInputBuffers[i] != NULL) {
      drv->nvencFuncs.nvEncDestroyInputBuffer(nvCtx->nvencEncoder,
                                              nvCtx->nvencInputBuffers[i]);
      nvCtx->nvencInputBuffers[i] = NULL;
    }
    if (nvCtx->nvencOutputBuffers[i] != NULL) {
      drv->nvencFuncs.nvEncDestroyBitstreamBuffer(nvCtx->nvencEncoder,
                                                  nvCtx->nvencOutputBuffers[i]);
...
static void nvencSetFrameRate(NVContext *nvCtx, uint32_t fpsNum,
                              uint32_t fpsDen) {
  if (fpsNum == 0) {
    return;
  }
...
static void nvencSetGop(NVContext *nvCtx, uint32_t gopLength,
                        uint32_t idrPeriod) {
  if (gopLength > 0) {
  ...
  if (!nvCtx->encIsHevc) {
    if (idrPeriod > 0) {
      if (setU32IfChanged(
              &nvCtx->encConfig.encodeCodecConfig.h264Config.idrPeriod,
              idrPeriod)) {
        nvCtx->encNeedsReconfigure = 1;
      }
    }
    ...
    if (idrPeriod > 0) {
      if (setU32IfChanged(
              &nvCtx->encConfig.encodeCodecConfig.hevcConfig.idrPeriod,
              idrPeriod)) {
        nvCtx->encNeedsReconfigure = 1;
      }
    }
...
static void nvencSetVisibleRect(NVContext *nvCtx, uint32_t x, uint32_t y,
...
static void nvencMaybeUseVisibleResolution(NVContext *nvCtx) {
  ...
  if (visW > nvCtx->encInitParams.maxEncodeWidth ||
      visH > nvCtx->encInitParams.maxEncodeHeight) {
    return;
  }
  ...
  nvCtx->encNeedsReconfigure = 1;
  LOG("NVENC visible reconfigure requested: %dx%d -> %ux%u", nvCtx->width,
      nvCtx->height, visW, visH);
}

static void
nvencUpdateVisibleRectFromH264Seq(NVContext *nvCtx,
...
static void
nvencUpdateVisibleRectFromHevcSeq(NVContext *nvCtx,
                                  const VAEncSequenceParameterBufferHEVC *seq) {
  if (!seq) {
    return;
  }
  if (seq->pic_width_in_luma_samples == 0 ||
      seq->pic_height_in_luma_samples == 0) {
    return;
  }
  nvencSetVisibleRect(nvCtx, 0, 0, seq->pic_width_in_luma_samples,
...
static VAStatus nvencReconfigureIfNeeded(NVDriver *drv, NVContext *nvCtx) {
  ...
  if (!resolutionChange && g_encReconfigureMinMs > 0) {
    ...
    if (nvCtx->encLastReconfigureNs != 0 && now > nvCtx->encLastReconfigureNs &&
  ...
  NVENCSTATUS nvs =
      drv->nvencFuncs.nvEncReconfigureEncoder(nvCtx->nvencEncoder, &reconfig);
  ...
  if (oldWidth != (uint32_t)nvCtx->width ||
...
static void *resolveSurfaces(void *param) {
  ...
  while (!ctx->exiting) {
    ...
    pthread_mutex_unlock(&ctx->resolveMutex);
    // find the last item
    // LOG("Reading from queue: %d %d", ctx->surfaceQueueReadIdx,
    ...
    // map frame
    CUVIDPROCPARAMS procParams = {.progressive_frame =
                                      surface->progressiveFrame,
                                  .top_field_first = surface->topFieldFirst,
                                  .second_field = surface->secondField};

    // LOG("Mapping surface %d", surface->pictureIdx);
    if (surface->decodeFailed || CHECK_CUDA_RESULT(cv->cuvidMapVideoFrame(
                                     ctx->decoder, surface->pictureIdx,
                                     &deviceMemory, &pitch, &procParams))) {
    ...
    // LOG("Mapped surface %d to %p (%d)", surface->pictureIdx,
    // (void*)deviceMemory, pitch);
...
static VAStatus nvQueryConfigProfiles(VADriverContextP ctx,
...
static VAStatus nvQueryConfigProfiles2(VADriverContextP ctx,
                                       VAProfile *profile_list, /* out */
                                       int *num_profiles        /* out */
) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  ...
  int profiles = 0;
  if (doesGPUSupportCodec(cudaVideoCodec_MPEG2, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileMPEG2Simple;
    profile_list[profiles++] = VAProfileMPEG2Main;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_MPEG4, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
  ...
  if (doesGPUSupportCodec(cudaVideoCodec_VC1, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileVC1Simple;
    profile_list[profiles++] = VAProfileVC1Main;
    profile_list[profiles++] = VAProfileVC1Advanced;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_H264, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
  ...
  if (doesGPUSupportCodec(cudaVideoCodec_JPEG, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileJPEGBaseline;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_H264_SVC, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileH264StereoHigh;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_H264_MVC, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileH264MultiviewHigh;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileHEVCMain;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_VP8, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileVP8Version0_3;
  }
  if (doesGPUSupportCodec(cudaVideoCodec_VP9, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileVP9Profile0; // color depth: 8 bit,
                                                     // 4:2:0
  }
  if (doesGPUSupportCodec(cudaVideoCodec_AV1, 8, cudaVideoChromaFormat_420,
                          NULL, NULL)) {
    profile_list[profiles++] = VAProfileAV1Profile0;
  }

  if (drv->supports16BitSurface) {
    if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 10, cudaVideoChromaFormat_420,
                            NULL, NULL)) {
      profile_list[profiles++] = VAProfileHEVCMain10;
    }
    if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 12, cudaVideoChromaFormat_420,
                            NULL, NULL)) {
      profile_list[profiles++] = VAProfileHEVCMain12;
    }
    if (doesGPUSupportCodec(cudaVideoCodec_VP9, 10, cudaVideoChromaFormat_420,
                            NULL, NULL)) {
      profile_list[profiles++] =
          VAProfileVP9Profile2; // color depth: 10–12 bit, 4:2:0
    }
  }

  if (drv->supports444Surface) {
    if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 8, cudaVideoChromaFormat_444,
                            NULL, NULL)) {
      profile_list[profiles++] = VAProfileHEVCMain444;
    }
    if (doesGPUSupportCodec(cudaVideoCodec_VP9, 8, cudaVideoChromaFormat_444,
                            NULL, NULL)) {
      profile_list[profiles++] =
          VAProfileVP9Profile1; // color depth: 8 bit, 4:2:2, 4:4:0, 4:4:4
    }
    if (doesGPUSupportCodec(cudaVideoCodec_AV1, 8, cudaVideoChromaFormat_444,
                            NULL, NULL)) {
...
#if VA_CHECK_VERSION(1, 20, 0)
    if (drv->supports16BitSurface) {
      if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 10,
                              cudaVideoChromaFormat_444, NULL, NULL)) {
        profile_list[profiles++] = VAProfileHEVCMain444_10;
      }
      if (doesGPUSupportCodec(cudaVideoCodec_HEVC, 12,
                              cudaVideoChromaFormat_444, NULL, NULL)) {
        profile_list[profiles++] = VAProfileHEVCMain444_12;
      }
      if (doesGPUSupportCodec(cudaVideoCodec_VP9, 10, cudaVideoChromaFormat_444,
                              NULL, NULL)) {
        profile_list[profiles++] =
            VAProfileVP9Profile3; // color depth: 10–12 bit, 4:2:2, 4:4:0, 4:4:4
      }
    }
#endif
  }
  ...
  *num_profiles = profiles;
  ...
  CHECK_CUDA_RESULT_RETURN(cu->cuCtxPopCurrent(NULL),
                           VA_STATUS_ERROR_OPERATION_FAILED);
...
static VAStatus
nvQueryConfigEntrypoints(VADriverContextP ctx, VAProfile profile,
...
static VAStatus nvGetConfigAttributes(VADriverContextP ctx, VAProfile profile,
                                      VAEntrypoint entrypoint,
                                      VAConfigAttrib *attrib_list,
                                      int num_attribs) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;

  /* ── Handle ENCODE entrypoints FIRST ── */
  ...
  for (int i = 0; i < num_attribs; i++) {
    if (attrib_list[i].type == VAConfigAttribRTFormat) {
      ...
      uint32_t width = 0;
      if (!doesGPUSupportCodec(vaToCuCodec(profile), 8,
                               cudaVideoChromaFormat_420, &width, NULL) ||
          width == 0) {
        width = 8192;
      }
      ...
      uint32_t height = 0;
      if (!doesGPUSupportCodec(vaToCuCodec(profile), 8,
                               cudaVideoChromaFormat_420, NULL, &height) ||
          height == 0) {
        height = 8192;
      }
...
static VAStatus nvCreateConfig(VADriverContextP ctx, VAProfile profile,
                               VAEntrypoint entrypoint,
  ...
  // for (int i = 0; i < num_attribs; i++) {
  //   LOG("got config attrib: %d %d %d", i, attrib_list[i].type,
  //   attrib_list[i].value);
  // }
  ...
  if (drv->supports16BitSurface) {
    ...
    case VAProfileVP9Profile2:
    case VAProfileAV1Profile0:
      // If the user provides an RTFormat, we can use that to identify which
      // decoder configuration is appropriate. If a format is not required here,
      // the caller must pass render targets to createContext so we can use
      // those to establish the surface format and bit depth.
      if (num_attribs > 0 && attrib_list[0].type == VAConfigAttribRTFormat) {
        ...
        if (cfg->profile == VAProfileVP9Profile2) {
          ...
          if (!g_warnedVp9Av1NoRtformat) {
            LOG("Unable to determine surface type for VP9/AV1 codec due to no "
                "RTFormat specified.");
...
static VAStatus nvDestroyConfig(VADriverContextP ctx, VAConfigID config_id) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
...
static VAStatus nvQueryConfigAttributes(VADriverContextP ctx,
  ...
  if (!drv->supports16BitSurface) {
    attrib_list[i].value &= ~(VA_RT_FORMAT_YUV420_10 | VA_RT_FORMAT_YUV420_12 |
                              VA_RT_FORMAT_YUV444_10 | VA_RT_FORMAT_YUV444_12);
  }
  if (!drv->supports444Surface) {
    attrib_list[i].value &= ~(VA_RT_FORMAT_YUV444 | VA_RT_FORMAT_YUV444_10 |
...
static VAStatus nvCreateSurfaces2(VADriverContextP ctx, unsigned int format,
  ...
  LOG("Creating %u surface(s): %ux%u format=%X", num_surfaces, width, height,
      format);
  for (uint32_t i = 0; i < num_surfaces; i++) {
    Object surfaceObject =
        allocateObject(drv, OBJECT_TYPE_SURFACE, sizeof(NVSurface));
...
static VAStatus nvCreateSurfaces(VADriverContextP ctx, int width, int height,
                                 int format, int num_surfaces,
                                 VASurfaceID *surfaces /* out */
) {
  return nvCreateSurfaces2(ctx, format, width, height, surfaces, num_surfaces,
                           NULL, 0);
}

static VAStatus nvDestroySurfaces(VADriverContextP ctx,
  ...
  for (int i = 0; i < num_surfaces; i++) {
    NVSurface *surface =
        (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, surface_list[i]);
    if (!surface) {
...
static VAStatus nvCreateContext(VADriverContextP ctx, VAConfigID config_id,
  ...
  if (cfg->isEncode) {
    ...
    Object contextObj =
        allocateObject(drv, OBJECT_TYPE_CONTEXT, sizeof(NVContext));
    ...
    if (!profileToNvencGuids(cfg->profile, &nvCtx->encCodecGuid,
                             &nvCtx->encProfileGuid, &nvCtx->encIsHevc)) {
    ...
    NVENCSTATUS nvs = drv->nvencFuncs.nvEncOpenEncodeSessionEx(
        &openParams, &nvCtx->nvencEncoder);
    if (nvs != NV_ENC_SUCCESS) {
    ...
    /* Use a more conservative default bitrate (2 Mbps as opposed to 5 Mbps) -
     * will be updated by the first VAEncSequenceParameterBufferH264 or
    ...
    if (!nvCtx->encIsHevc) {
      NV_ENC_CONFIG_H264 *h264 = &nvCtx->encConfig.encodeCodecConfig.h264Config;

      h264->idrPeriod = nvCtx->encConfig.gopLength;
      h264->repeatSPSPPS = 1;  /* SPS/PPS with each IDR */
      h264->disableSPSPPS = 0; /* Don't disable SPS/PPS */
      h264->outputAUD = 0;     /* WebRTC doesn't want Access Unit Delimiters */
      h264->chromaFormatIDC = 1; /* 4:2:0 */
      h264->maxNumRefFrames = 1; /* Low-latency: 1 ref frame */
      h264->sliceMode = 0;       /* One slice per frame */
      h264->sliceModeData = 0;

      h264->outputBufferingPeriodSEI = 0; /* Not necessary for WebRTC */
      h264->outputPictureTimingSEI = 0;

      /* Constrained Baseline requires CAVLC (no CABAC!) */
      if (nvCtx->profile == VAProfileH264ConstrainedBaseline ||
          nvCtx->encProfileGuid.Data1 ==
              NV_ENC_H264_PROFILE_BASELINE_GUID.Data1) {
        h264->entropyCodingMode = NV_ENC_H264_ENTROPY_CODING_MODE_CAVLC;
      }
      /* Main/High can use CABAC */
      else {
        h264->entropyCodingMode = NV_ENC_H264_ENTROPY_CODING_MODE_CABAC;
      }
    } else {
      NV_ENC_CONFIG_HEVC *hevc = &nvCtx->encConfig.encodeCodecConfig.hevcConfig;
      hevc->idrPeriod = nvCtx->encConfig.gopLength;
      hevc->repeatSPSPPS = 1;
    }
    ...
    for (uint32_t i = 0; i < nvCtx->nvencIoBufferCount; i++) {
      ...
      nvs = drv->nvencFuncs.nvEncCreateBitstreamBuffer(nvCtx->nvencEncoder,
                                                       &createBS);
      if (nvs != NV_ENC_SUCCESS) {
        nvencDestroyIoBuffers(drv, nvCtx);
        drv->nvencFuncs.nvEncDestroyEncoder(nvCtx->nvencEncoder);
        ...
        cu->cuCtxPopCurrent(NULL);
        deleteObject(drv, contextObj->id);
        LOG("nvEncCreateBitstreamBuffer[%u] failed: %s", i,
            nvencStatusStr(nvs));
        return nvencToVaStatus(nvs);
      }
  ...
  LOG("Creating context with %d render targets, at %dx%d", num_render_targets,
  ...
  if (num_render_targets) {
    ...
    NVSurface *surface =
        (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, render_targets[0]);
    if (!surface) {
      return VA_STATUS_ERROR_INVALID_PARAMETER;
    }
  ...
  // Setting to maximun value if num_render_targets == 0 to prevent picture
  // index overflow as additional surfaces can be created after calling
  // nvCreateContext
  int surfaceCount = num_render_targets > 0 ? num_render_targets : 32;

  if (surfaceCount > 32) {
    LOG("Application requested %d surface(s), limiting to 32. This may cause "
        "issues.",
        surfaceCount);
    surfaceCount = 32;
  }
  ...
  CUVIDDECODECREATEINFO vdci = {
      ...
      // just allocate as many surfaces as have been created since we can never
      // have as much information as the decode to guess correctly
  ...
  CHECK_CUDA_RESULT_RETURN(cv->cuvidCreateDecoder(&decoder, &vdci),
  ...
  int err =
      pthread_create(&nvCtx->resolveThread, NULL, &resolveSurfaces, nvCtx);
  if (err != 0) {
...
static VAStatus nvDestroyContext(VADriverContextP ctx, VAContextID context) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  LOG("Destroying context: %d", context);

  NVContext *nvCtx =
      (NVContext *)getObjectPtr(drv, OBJECT_TYPE_CONTEXT, context);
...
static VAStatus nvCreateBuffer(VADriverContextP ctx,
                               ...
                               void *data,                /* in */
                               VABufferID *buf_id) {
  // LOG("got buffer %p, type %x, size %u, elements %u", data, type, size,
  // num_elements);
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  ...
  // HACK: This is an awful hack to support VP8 videos when running within
  // FFMPEG. VA-API doesn't pass enough information for NVDEC to work with, but
  // the information is there just before the start of the buffer that was
  // passed to us.
  size_t offset = 0;
  if (!isEncode && nvCtx != NULL && nvCtx->profile == VAProfileVP8Version0_3 &&
...
static VAStatus nvBufferSetNumElements(VADriverContextP ctx,
...
static VAStatus nvMapBuffer(VADriverContextP ctx, VABufferID buf_id, /* in */
                            void **pbuf                              /* out */
) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  ...
  if (buf->bufferType == VAImageBufferType &&
    ...
    if (surf != NULL && ensureSurfaceHostBuffer(surf) && g_encStrictMode) {
      ...
      if (!buf->ownsPtr || buf->ptr == surf->hostData || buf->size != need ||
          buf->ptr == NULL) {
        uint8_t *shadow =
            realloc((buf->ownsPtr && buf->ptr != NULL) ? buf->ptr : NULL, need);
        if (shadow == NULL) {
          return VA_STATUS_ERROR_ALLOCATION_FAILED;
        }
        buf->ptr = shadow;
  ...
  if (buf->bufferType == VAImageBufferType &&
      buf->surfaceId != VA_INVALID_SURFACE && !buf->mappedSurfaceLock) {
    NVSurface *surf =
        (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, buf->surfaceId);
    if (surf != NULL) {
      pthread_mutex_lock(&surf->mutex);
...
static VAStatus nvDestroyBuffer(VADriverContextP ctx, VABufferID buffer_id) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
...
static VAStatus nvBeginPicture(VADriverContextP ctx, VAContextID context,
                               VASurfaceID render_target) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  NVContext *nvCtx =
      (NVContext *)getObjectPtr(drv, OBJECT_TYPE_CONTEXT, context);
  NVSurface *surface =
      (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, render_target);
  ...
  if (surface->context != NULL && surface->context != nvCtx) {
    // this surface was last used on a different context, we need to free up the
    // backing image (it might not be the correct size)
  ...
  nvCtx->renderTarget->progressiveFrame =
      true; // assume we're producing progressive frame unless the codec says
            // otherwise
...
static VAStatus nvRenderPicture(VADriverContextP ctx, VAContextID context,
  ...
  if (nvCtx->mode == NV_CONTEXT_ENCODE) {
    for (int i = 0; i < num_buffers; i++) {
      NVBuffer *buf =
          (NVBuffer *)getObjectPtr(drv, OBJECT_TYPE_BUFFER, buffers[i]);
      if (buf == NULL) {
        continue;
      }
      ...
      case VAEncSequenceParameterBufferType:
        if (!nvCtx->encIsHevc && buf->ptr != NULL &&
            buf->size >= sizeof(VAEncSequenceParameterBufferH264)) {
          const VAEncSequenceParameterBufferH264 *seq =
              (const VAEncSequenceParameterBufferH264 *)buf->ptr;
          ...
          if (seq->vui_fields.bits.timing_info_present_flag &&
              seq->time_scale != 0) {
            uint32_t den =
                seq->num_units_in_tick ? (seq->num_units_in_tick * 2U) : 1U;
          ...
          const VAEncSequenceParameterBufferHEVC *seq =
              (const VAEncSequenceParameterBufferHEVC *)buf->ptr;
          nvencSetBitrate(nvCtx, seq->bits_per_second);
          nvencSetGop(nvCtx, seq->intra_period, seq->intra_idr_period);
          ...
          if (seq->vui_fields.bits.vui_timing_info_present_flag &&
            ...
            uint32_t den =
                seq->vui_num_units_in_tick ? seq->vui_num_units_in_tick : 1U;
      ...
      case VAEncPictureParameterBufferType:
        if (!nvCtx->encIsHevc && buf->ptr != NULL &&
          ...
          const VAEncPictureParameterBufferH264 *pic =
              (const VAEncPictureParameterBufferH264 *)buf->ptr;
          if (pic->coded_buf != VA_INVALID_ID) {
            NVBuffer *codedBuf = (NVBuffer *)getObjectPtr(
                drv, OBJECT_TYPE_BUFFER, pic->coded_buf);
            if (codedBuf != NULL &&
                codedBuf->bufferType == VAEncCodedBufferType) {
          ...
          const VAEncPictureParameterBufferHEVC *pic =
              (const VAEncPictureParameterBufferHEVC *)buf->ptr;
          if (pic->coded_buf != VA_INVALID_ID) {
            NVBuffer *codedBuf = (NVBuffer *)getObjectPtr(
                drv, OBJECT_TYPE_BUFFER, pic->coded_buf);
            if (codedBuf != NULL &&
      ...
      case VAEncMiscParameterBufferType:
        if (buf->ptr != NULL && buf->size >= sizeof(VAEncMiscParameterBuffer)) {
          const VAEncMiscParameterBuffer *misc =
              (const VAEncMiscParameterBuffer *)buf->ptr;
          ...
          if (misc->type == VAEncMiscParameterTypeRateControl &&
            ...
            const VAEncMiscParameterRateControl *rc =
                (const VAEncMiscParameterRateControl *)payload;
            if (rc->bits_per_second > 0) {
              if (rc->target_percentage &&
                  nvCtx->encConfig.rcParams.rateControlMode ==
                      NV_ENC_PARAMS_RC_VBR) {
                uint64_t avg = ((uint64_t)rc->bits_per_second *
                                (uint64_t)rc->target_percentage) /
                               100ULL;
                if (avg > UINT32_MAX) {
                  avg = UINT32_MAX;
                }
                nvencSetBitrate(nvCtx, (uint32_t)avg);
                if (setU32IfChanged(&nvCtx->encConfig.rcParams.maxBitRate,
                                    rc->bits_per_second)) {
                  nvCtx->encNeedsReconfigure = 1;
                }
              } else {
            ...
            const VAEncMiscParameterFrameRate *fr =
                (const VAEncMiscParameterFrameRate *)payload;
...
static VAStatus nvEndPicture(VADriverContextP ctx, VAContextID context) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  NVContext *nvCtx =
      (NVContext *)getObjectPtr(drv, OBJECT_TYPE_CONTEXT, context);
  ...
  if (nvCtx->mode == NV_CONTEXT_ENCODE) {
    ...
    NVSurface *surface = (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE,
                                                   nvCtx->encCurrentSurface);
    NVBuffer *codedBuf =
        (NVBuffer *)getObjectPtr(drv, OBJECT_TYPE_BUFFER, nvCtx->encCodedBufId);
    ...
    NVENCSTATUS nvs =
        drv->nvencFuncs.nvEncLockInputBuffer(nvCtx->nvencEncoder, &lockInput);
    if (nvs != NV_ENC_SUCCESS) {
      cu->cuCtxPopCurrent(NULL);
      surface->status = VASurfaceReady;
    ...
    uint32_t copyHeight =
        surface->height < encHeight ? surface->height : encHeight;
    ...
    if (nvCtx->encVisibleValid && nvCtx->encVisibleWidth > 0 &&
      ...
      copyWidth =
          nvCtx->encVisibleWidth < encWidth ? nvCtx->encVisibleWidth : encWidth;
      copyHeight = nvCtx->encVisibleHeight < encHeight ? nvCtx->encVisibleHeight
                                                       : encHeight;
      ...
      copyWidth =
          surface->contentWidth < encWidth ? surface->contentWidth : encWidth;
      copyHeight = surface->contentHeight < encHeight ? surface->contentHeight
                                                      : encHeight;
    ...
    if (srcX == 0 && srcY == 0 && copyWidth == encWidth &&
      ...
      memcpy(dstUV, src + surface->hostYSize,
             ((size_t)encHeight / 2U) * dstPitch);
    } else {
      /* Copy luma/chroma payload first. */
      for (uint32_t y = 0; y < copyHeight; y++) {
        memcpy(dst + ((size_t)y * dstPitch), srcYBase + ((size_t)y * srcPitch),
               copyWidth);
      }
      for (uint32_t y = 0; y < (copyHeight / 2U); y++) {
        memcpy(dstUV + ((size_t)y * dstPitch), srcUV + ((size_t)y * srcPitch),
               copyWidth);
      }
    ...
    if (encDbgEnabled) {
      ...
      LOG("ENCDBG pre frame=%u hash_full=%016llx hash_top32=%016llx copy=%ux%u "
          "enc=%ux%u src=(%u,%u) pitch=%zu",
          encDbgFrame, (unsigned long long)encDbgPreFullHash,
          (unsigned long long)encDbgPreTopHash, copyWidth, copyHeight, encWidth,
          encHeight, srcX, srcY, dstPitch);
    ...
    nvs = drv->nvencFuncs.nvEncUnlockInputBuffer(nvCtx->nvencEncoder,
                                                 inputBuffer);
    if (nvs != NV_ENC_SUCCESS) {
      cu->cuCtxPopCurrent(NULL);
      surface->status = VASurfaceReady;
    ...
    if (nvs == NV_ENC_ERR_INVALID_PARAM &&
      ...
      LOG("nvEncEncodePicture rejected dynamic input size %ux%u (ctx %dx%d), "
          "trying reconfigure",
          picParams.inputWidth, picParams.inputHeight, nvCtx->width,
          nvCtx->height);
      ...
      if (drv->nvencFuncs.nvEncReconfigureEncoder) {
        ...
        NVENCSTATUS rnvs = drv->nvencFuncs.nvEncReconfigureEncoder(
            nvCtx->nvencEncoder, &reconfig);
        if (rnvs == NV_ENC_SUCCESS) {
          ...
          LOG("nvEncReconfigureEncoder resolution change failed: %s",
              nvencStatusStr(rnvs));
        }
      }
      ...
      if (reconfigured) {
        nvs =
            drv->nvencFuncs.nvEncEncodePicture(nvCtx->nvencEncoder, &picParams);
      }
      if (nvs == NV_ENC_ERR_INVALID_PARAM) {
        picParams.inputWidth = (uint32_t)nvCtx->width;
        picParams.inputHeight = (uint32_t)nvCtx->height;
        LOG("Falling back to context encode size %ux%u", picParams.inputWidth,
    ...
    if (nvs == NV_ENC_ERR_NEED_MORE_INPUT) {
      ...
      if (nvCtx->encNeedMoreInputStreak == 1 ||
          (nvCtx->encNeedMoreInputStreak % 30U) == 0U) {
    ...
    if (encDbgEnabled) {
      ...
      if (lockBS.bitstreamBufferPtr != NULL &&
          lockBS.bitstreamSizeInBytes > 0) {
        bitstreamHash = fnv1a64_append(
            bitstreamHash, (const uint8_t *)lockBS.bitstreamBufferPtr,
            (size_t)lockBS.bitstreamSizeInBytes);
      }
      LOG("ENCDBG bitstream frame=%u size=%u hash=%016llx pre_full=%016llx "
          "pre_top32=%016llx flags=0x%x",
          encDbgFrame, lockBS.bitstreamSizeInBytes,
          (unsigned long long)bitstreamHash,
          (unsigned long long)encDbgPreFullHash,
          (unsigned long long)encDbgPreTopHash, picParams.encodePicFlags);

      if (encDbgDump) {
        char leaf[128];
        char path[PATH_MAX];
        const char *ext = nvCtx->encIsHevc ? "h265" : "h264";
        snprintf(leaf, sizeof(leaf), "frame_%06u_bitstream.%s", encDbgFrame,
                 ext);
        if (nvencDebugBuildPath(path, sizeof(path), leaf)) {
          nvencDebugDumpBytes(path, (const uint8_t *)lockBS.bitstreamBufferPtr,
                              (size_t)lockBS.bitstreamSizeInBytes);
        }
      }
    }
    ...
    nvs =
        drv->nvencFuncs.nvEncUnlockBitstream(nvCtx->nvencEncoder, outputBuffer);
    if (nvs != NV_ENC_SUCCESS) {
    ...
    if (nvCtx->encOutputFrameCount == 1) {
      nvencHealthLog("INFO", "HW encode active (%s %ux%u)",
                     nvCtx->encIsHevc ? "HEVC" : "H264",
                     (unsigned int)copyWidth, (unsigned int)copyHeight);
      nvCtx->encLastHealthLogFrame = 1;
    } else if (nvCtx->encOutputFrameCount >=
               nvCtx->encLastHealthLogFrame + 900U) {
      nvencHealthLog("INFO", "HW encode still active (%s frames=%u)",
                     nvCtx->encIsHevc ? "HEVC" : "H264",
  ...
  // LOG("Decoded frame successfully to idx: %d (%p)", picParams->CurrPicIdx,
  // nvCtx->renderTarget);
...
static VAStatus nvQuerySurfaceStatus(VADriverContextP ctx,
                                     VASurfaceID render_target,
...
static VAStatus nvQuerySurfaceError(VADriverContextP ctx,
                                    VASurfaceID render_target,
...
static VAStatus nvQueryImageFormats(VADriverContextP ctx,
...
static VAStatus nvCreateImage(VADriverContextP ctx, VAImageFormat *format,
  ...
  // TODO could probably put these in a pool, they appear to be allocated, used,
  // then freed
  ...
  for (uint32_t i = 0; i < fmtInfo->numPlanes; i++) {
    imageBuffer->size += ((width * height) >> (p[i].ss.x + p[i].ss.y)) *
   ...
   * allocated based on the image size and format. This buffer is managed by the
   * library implementation, and accessed by the client through the buffer
   * Map/Unmap functions.
   */
  image->width = width;
  image->height = height;
  ...
  image->offsets[0] = 0;
  image->offsets[1] =
      image->offsets[0] + ((width * height) >> (p[0].ss.x + p[0].ss.y)) *
                              fmtInfo->bppc * p[0].channelCount;
  image->offsets[2] =
      image->offsets[1] + ((width * height) >> (p[1].ss.x + p[1].ss.y)) *
                              fmtInfo->bppc * p[1].channelCount;
...
static VAStatus nvDeriveImage(VADriverContextP ctx, VASurfaceID surface,
                              VAImage *image /* out */
) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  NVSurface *surfaceObj =
      (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, surface);
  if (surfaceObj == NULL) {
    return VA_STATUS_ERROR_INVALID_SURFACE;
  }
...
static VAStatus nvDestroyImage(VADriverContextP ctx, VAImageID image) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  ...
  Object imageBufferObj =
      getObjectByPtr(drv, OBJECT_TYPE_BUFFER, img->imageBuffer);
...
static VAStatus
nvGetImage(VADriverContextP ctx, VASurfaceID surface,
  ...
  if (context == NULL || context->mode == NV_CONTEXT_ENCODE) {
    if (!surfaceObj->hostData || !imageObj->imageBuffer ||
    ...
    uint32_t dstPitchY =
        imageObj->pitches[0] ? imageObj->pitches[0] : imageObj->width;
    uint32_t dstPitchUV =
        imageObj->pitches[1] ? imageObj->pitches[1] : dstPitchY;
    ...
    for (uint32_t row = 0; row < uvHeight; row++) {
      memcpy(dst + dstOffsetUV + ((size_t)row * dstPitchUV),
             srcUV + ((size_t)row * surfaceObj->hostPitch), uvWidth);
    }
  ...
  for (uint32_t i = 0; i < fmtInfo->numPlanes; i++) {
    ...
    offset += ((width * height) >> (p->ss.x + p->ss.y)) * fmtInfo->bppc *
              p->channelCount;
  }
  CHECK_CUDA_RESULT_RETURN(cu->cuCtxPopCurrent(NULL),
                           VA_STATUS_ERROR_OPERATION_FAILED);
...
static VAStatus nvPutImage(VADriverContextP ctx, VASurfaceID surface,
  ...
  if ((uint32_t)src_x >= imageObj->width ||
      (uint32_t)src_y >= imageObj->height) {
    return VA_STATUS_ERROR_INVALID_PARAMETER;
  }
  if ((uint32_t)dest_x >= surfaceObj->width ||
      (uint32_t)dest_y >= surfaceObj->height) {
    return VA_STATUS_ERROR_INVALID_PARAMETER;
  }
  ...
  pthread_mutex_lock(&surfaceObj->mutex);
  const uint8_t *srcY =
      srcBase + srcOffsetY + ((size_t)src_y * srcPitchY) + (size_t)src_x;
  uint8_t *dstY = surfaceObj->hostData +
                  ((size_t)dest_y * surfaceObj->hostPitch) + (size_t)dest_x;

  for (uint32_t row = 0; row < copyH; row++) {
    memcpy(dstY + ((size_t)row * surfaceObj->hostPitch),
           srcY + ((size_t)row * srcPitchY), copyW);
  }
  ...
  const uint8_t *srcUV = srcBase + srcOffsetUV +
                         ((size_t)((uint32_t)src_y / 2U) * srcPitchUV) +
                         (size_t)srcXUV;
  uint8_t *dstUV = surfaceObj->hostData + surfaceObj->hostYSize +
                   ((size_t)((uint32_t)dest_y / 2U) * surfaceObj->hostPitch) +
                   (size_t)dstXUV;

  for (uint32_t row = 0; row < uvHeight; row++) {
    memcpy(dstUV + ((size_t)row * surfaceObj->hostPitch),
           srcUV + ((size_t)row * srcPitchUV), uvWidth);
  }
  ...
  if (copyW > 0 && copyH > 0) {
    ...
    if (!surfaceObj->contentValid) {
      ...
      surfaceObj->contentValid =
          (surfaceObj->contentWidth > 0 && surfaceObj->contentHeight > 0) ? 1
                                                                          : 0;
    }
  }
  pthread_mutex_unlock(&surfaceObj->mutex);
...
static VAStatus nvQuerySubpictureFormats(VADriverContextP ctx,
                                         VAImageFormat *format_list, /* out */
...
static VAStatus nvCreateSubpicture(VADriverContextP ctx, VAImageID image,
...
static VAStatus nvDestroySubpicture(VADriverContextP ctx,
                                    VASubpictureID subpicture) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
}

static VAStatus nvSetSubpictureImage(VADriverContextP ctx,
                                     VASubpictureID subpicture,
                                     VAImageID image) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
...
static VAStatus nvSetSubpictureChromakey(VADriverContextP ctx,
                                         VASubpictureID subpicture,
...
static VAStatus nvSetSubpictureGlobalAlpha(VADriverContextP ctx,
                                           VASubpictureID subpicture,
                                           float global_alpha) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
...
static VAStatus nvAssociateSubpicture(
    ...
    short src_y, unsigned short src_width, unsigned short src_height,
...
static VAStatus nvDeassociateSubpicture(VADriverContextP ctx,
                                        VASubpictureID subpicture,
...
static VAStatus
nvQueryDisplayAttributes(VADriverContextP ctx,
...
static VAStatus
nvGetDisplayAttributes(VADriverContextP ctx,
...
static VAStatus nvSetDisplayAttributes(VADriverContextP ctx,
...
static VAStatus nvQuerySurfaceAttributes(VADriverContextP ctx,
                                         VAConfigID config,
                                         VASurfaceAttrib *attrib_list,
                                         unsigned int *num_attribs) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  ...
  // LOG("with %d (%d) %p %d", cfg->cudaCodec, cfg->bitDepth, attrib_list,
  // *num_attribs);
  ...
  CUVIDDECODECAPS videoDecodeCaps = {.eCodecType = cfg->cudaCodec,
                                     .eChromaFormat = cfg->chromaFormat,
                                     .nBitDepthMinus8 = cfg->bitDepth - 8};
  bool decodeCapsValid = true;
  if (checkCudaErrors(cu->cuCtxPushCurrent(drv->cudaContext), __FILE__,
                      __func__, __LINE__)) {
    decodeCapsValid = false;
  } else {
    if (checkCudaErrors(cv->cuvidGetDecoderCaps(&videoDecodeCaps), __FILE__,
                        __func__, __LINE__)) {
      decodeCapsValid = false;
    }
    if (checkCudaErrors(cu->cuCtxPopCurrent(NULL), __FILE__, __func__,
                        __LINE__)) {
      decodeCapsValid = false;
    }
  }
  if (!decodeCapsValid || videoDecodeCaps.nMaxWidth == 0 ||
      videoDecodeCaps.nMaxHeight == 0) {
...
/* used by va trace */
static VAStatus nvBufferInfo(VADriverContextP ctx,      /* in */
                             VABufferID buf_id,         /* in */
...
static VAStatus nvAcquireBufferHandle(VADriverContextP ctx,
                                      VABufferID buf_id,     /* in */
...
static VAStatus nvReleaseBufferHandle(VADriverContextP ctx,
                                      VABufferID buf_id /* in */
) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
...
static VAStatus
nvLockSurface(VADriverContextP ctx, VASurfaceID surface,
              ...
              unsigned int *buffer_name, /* if it is not NULL, assign the low
                                          * lever surface buffer name
                                          */
  ...
  if (luma_stride)
    *luma_stride = (unsigned int)surf->hostPitch;
  if (chroma_u_stride)
    *chroma_u_stride = (unsigned int)surf->hostPitch;
  if (chroma_v_stride)
    *chroma_v_stride = (unsigned int)surf->hostPitch;
  if (luma_offset)
    *luma_offset = 0;
  if (chroma_u_offset)
    *chroma_u_offset = (unsigned int)surf->hostYSize;
  if (chroma_v_offset)
    *chroma_v_offset = (unsigned int)(surf->hostYSize + 1);
  if (buffer_name)
    *buffer_name = 0;
  if (buffer)
    *buffer = surf->hostData;
  return VA_STATUS_SUCCESS;
}

static VAStatus nvUnlockSurface(VADriverContextP ctx, VASurfaceID surface) {
  NVDriver *drv = (NVDriver *)ctx->pDriverData;
  NVSurface *surf =
      (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, surface);
  if (!surf) {
...
static VAStatus nvCreateMFContext(VADriverContextP ctx,
...
static VAStatus nvMFAddContext(VADriverContextP ctx, VAMFContextID mf_context,
                               VAContextID context) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
}

static VAStatus nvMFReleaseContext(VADriverContextP ctx,
...
static VAStatus nvMFSubmit(VADriverContextP ctx, VAMFContextID mf_context,
                           VAContextID *contexts, int num_contexts) {
  LOG("In %s", __func__);
  return VA_STATUS_ERROR_UNIMPLEMENTED;
}
static VAStatus nvCreateBuffer2(VADriverContextP ctx,
                                VAContextID context,     /* in */
                                VABufferType type,       /* in */
...
static VAStatus
nvQueryProcessingRate(VADriverContextP ctx,                /* in */
...
static VAStatus nvExportSurfaceHandle(VADriverContextP ctx,
  ...
  NVSurface *surface =
      (NVSurface *)getObjectPtr(drv, OBJECT_TYPE_SURFACE, surface_id);
  if (surface == NULL) {
    return VA_STATUS_ERROR_INVALID_SURFACE;
  }
  ...
  // LOG("Exporting with w:%d h:%d o:%d p:%d m:%" PRIx64 " o:%d p:%d m:%"
  // PRIx64, ptr->width, ptr->height, ptr->layers[0].offset[0],
  //                                                             ptr->layers[0].pitch[0],
  //                                                             ptr->objects[0].drm_format_modifier,
  //                                                             ptr->layers[1].offset[0],
  //                                                             ptr->layers[1].pitch[0],
...
static VAStatus nvTerminate(VADriverContextP ctx) {
  ...
  CHECK_CUDA_RESULT_RETURN(cu->cuCtxDestroy(drv->cudaContext),
                           VA_STATUS_ERROR_OPERATION_FAILED);
...
__vaDriverInit_1_0(VADriverContextP ctx) {
  ...
  // drm_state can be passed in with any display type, including X11. But if
  // it's X11, we don't want to use the fd as it'll likely be an Intel GPU, as
  // NVIDIA doesn't support DRI3 at the moment
  bool isDrm =
      ctx->drm_state != NULL && ((struct drm_state *)ctx->drm_state)->fd > 0;
  int drmFd =
      (gpu == -1 && isDrm) ? ((struct drm_state *)ctx->drm_state)->fd : -1;
  ...
  drv->cudaGpuId = gpu;
  // make sure that we want the default GPU, and that a DRM fd that we care
  // about is passed in
  drv->drmFd = drmFd;
  ...
  if (CHECK_CUDA_RESULT(cu->cuCtxCreate(
          &drv->cudaContext, CU_CTX_SCHED_BLOCKING_SYNC, drv->cudaGpuId))) {
    drv->backend->releaseExporter(drv);
    free(drv);
    return VA_STATUS_ERROR_OPERATION_FAILED;
  }
  ...
  // CHECK_CUDA_RESULT_RETURN(cv->cuvidCtxLockCreate(&drv->vidLock,
  // drv->cudaContext), VA_STATUS_ERROR_OPERATION_FAILED);

  VAStatus profileStatus =
